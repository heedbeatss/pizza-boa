...
cs = content.find('var CARDAPIO')
ce = content.find('};', cs)
cardapio = content[cs:ce+2]

print(f"HTML: {len(html_part)}, CARDAPIO: {len(cardapio)}")

# Verificar balanceamento do CARDAPIO
o = cardapio.count('{')
c = cardapio.count('}')
print(f"CARDAPIO: {o} abertas, {c} fechadas, diff={o-c}")

# Se CARDAPIO nao esta balanceado, corrigir
if o != c:
    # Adicionar chaves faltantes no final
    diff = o - c
    if diff > 0:
        cardapio = cardapio + '\n' + '}' * diff
        print(f"Adicionado {diff} chaves no CARDAPIO")
...
# Validar
with open('/tmp/test_validate.js', 'w') as f:
    f.write(js)
result = subprocess.run(['node', '-c', '/tmp/test_validate.js'], capture_output=True, text=True)
