...
var ZONAS_ENTREGA = [
  ...
  { nome: "Monsenhor Martinho", bairros: ["monsenhor martinho","martinho"], dist: 5, taxa: 8 },
  { nome: "Campestre", bairros: ["campestre"], dist: 6, taxa: 9 },
  { nome: "Tupi", bairros: ["tupi","tupy"], dist: 6.5, taxa: 9.5 },
  ...
  { nome: "Alfredo Guedes", bairros: ["alfredo guedes"], dist: 8, taxa: 11 },
  { nome: "Jardim Gloria", bairros: ["jardim gloria","jd. gloria"], dist: 8.5, taxa: 11.5 },
  { nome: "Esplanada", bairros: ["esplanada"], dist: 9, taxa: 12 },
  { nome: "Ceu Azul", bairros: ["ceu azul","céu azul"], dist: 7, taxa: 10 },
  { nome: "Vila Monteiro", bairros: ["vila monteiro","monteiro"], dist: 8, taxa: 11 },
  { nome: "Geraldo", bairros: ["geraldao","geraldão"], dist: 9, taxa: 12 },
  { nome: "Potumirim", bairros: ["potumirim"], dist: 10, taxa: 13 },
  { nome: "Santa Teresa", bairros: ["santa teresa","sta. teresa"], dist: 11, taxa: 14 },
...
function openItem(catId, itemId) {
  ...
  var h = '';
  h += '<div class="modal-header"><div><h2>' + item.nome + '</h2><p>' + item.descricao + '</p></div><button class="modal-close" onclick="closeModal(\\'' + itemModal\\')">✕</button></div>';
...
function setMetade(val) {
  ...
  if (val) {
    var h = '<div class="metade-info"><div class="metade-row"><span>1a metade: <strong>' + currentItem.nome + '</strong></span><span style="color:#a855f7">R$ ' + selTamanho.preco.toFixed(2) + '</span></div><div style="font-size:12px;color:#a1a1a1">Escolha a 2a metade:</div><div style="font-size:11px;color:#f59e0b;margin-top:4px">O preco final sera da metade mais cara</div></div>';
...
function calcPrice() {
  ...
  if (selMetade && selSegundo) {
    ...
    if (s) { var sp = s.tamanhos.find(function(t) { return t.nome === selTamanho.nome; }) || s.tamanhos[0]; preco = Math.max(preco, sp.preco); }
  }
  if (selBorda) preco += 8;
  return preco * selQty;
}
...
function updateCartButton() {
  ...
  if (count > 0) { btn.style.display = 'block'; document.getElementById('cartCount').textContent = count + ' item' + (count > 1 ? 's' : ''); document.getElementById('cartTotal').textContent = 'R$ ' + total.toFixed(2); }
  else { btn.style.display = 'none'; }
}

function toggleCart() {
  if (!isPizzariaAberta()) { alert('Estamos fechados!\\n\\nHorario: Terca a Domingo, 18h-01h.'); return; }
...
function renderCart() {
  ...
  for (var i = 0; i < cart.length; i++) {
    var it = cart[i];
    h += '<div class="cart-item"><div class="cart-item-info"><div class="name">' + it.qty + 'x ' + it.nome + ' (' + it.tamanho + ')</div><div class="price">R$ ' + (it.preco * it.qty).toFixed(2) + '</div></div><button onclick="removeCartItem(' + i + ')" style="background:none;border:none;color:#f87171;font-size:18px;cursor:pointer">✕</button></div>';
...
function renderCheckoutSummary() {
  ...
  if (zonaAtual) {
    h += '<div class="cart-summary-row"><span class="lbl">Entrega (' + zonaAtual.nome + ')</span><span style="color:' + (zonaAtual.valor === 0 ? '#4ade80' : '#a855f7') + '">' + (zonaAtual.valor === 0 ? 'GRATIS' : 'R$ ' + zonaAtual.valor.toFixed(2)) + '</span></div>';
  }
  if (total > 0 && total < MIN_PEDIDO_GRATIS && zonaAtual && zonaAtual.valor > 0) {
    h += '<div style="margin-top:8px;padding:8px;background:rgba(74,222,128,.1);border-radius:8px;font-size:12px;color:#4ade80;text-align:center">Falta R$ ' + (MIN_PEDIDO_GRATIS - total).toFixed(2) + ' para entrega GRATIS!</div>';
  }
...
function buscarCEP() {
  ...
  var status = document.getElementById('cepStatus');
  status.style.display = 'block'; status.textContent = 'Buscando...'; status.style.color = '#a1a1a1';
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === 4 && xhr.status === 200) {
      try {
        ...
        status.textContent = data.logradouro + ', ' + data.bairro; status.style.color = '#4ade80';
        document.getElementById('cep').value = cep.substring(0,5) + '-' + cep.substring(5);
        calcularEntrega();
        document.getElementById('numero').focus();
      } catch(e) { status.textContent = 'Erro'; status.style.color = '#f87171'; }
    }
  };
  xhr.send();
}
...
function submitOrder() {
  ...
  var pedido = {
    ...
    nome: nome,
    telefone: telefone,
    endereco: { cep: cep, rua: rua, numero: numero, bairro: bairro, cidade: cidade, complemento: complemento, referencia: referencia },
    itens: cart.slice(),
    pagamento: pay,
    troco: troco,
    total: total,
  ...
  msg += '🛵 *Entrega:* ' + (entrega === 0 ? 'GRATIS' : 'R$ ' + entrega.toFixed(2)) + '%0A';
...
function carregarPedidos() {
  var telefone = '';
  var elTel = document.getElementById('telefonePedidos');
  if (elTel) telefone = elTel.value.trim();
  if (!telefone) telefone = localStorage.getItem('ultimoTelefone') || '';
  if (!telefone) { var el = document.getElementById('meusPedidos'); if (el) el.innerHTML = '<div style="text-align:center;color:#a1a1a1;padding:20px">Digite seu telefone acima</div>'; return; }
  ...
  for (var i = meus.length - 1; i >= 0; i--) {
    ...
    h += '<strong style="font-size:16px">' + p.id + '</strong>';
    h += '<span class="status-badge ' + st.class + '">' + st.icon + ' ' + st.nome + '</span></div>';
    ...
    h += '</div>';
    h += '<div style="display:flex;justify-content:space-between;border-top:1px solid rgba(88,28,135,.2);padding-top:8px">';
    h += '<span style="color:#a1a1a1;font-size:12px">Entrega: ' + (p.entrega === 0 ? 'GRATIS' : 'R$ ' + p.entrega.toFixed(2)) + '</span>';
...
function carregarAdminPedidos() {
  ...
  for (var i = pedidos.length - 1; i >= 0; i--) {
    ...
    h += '<div style="font-size:13px;margin-top:4px;color:#a855f7">R$ ' + p.totalFinal.toFixed(2) + '</div></div>';
  }
  el.innerHTML = h;
}
...
function verPedidoAdmin(id) {
  ...
  h += '<div style="background:#0a0a0a;border-radius:12px;padding:16px;margin-bottom:16px">';
  h += '<h4 style="color:#a855f7;margin-bottom:8px">👤 Cliente</h4>';
  h += '<p><strong>Nome:</strong> ' + p.nome + '</p>';
  h += '<p><strong>Telefone:</strong> ' + p.telefone + '</p>';
  h += '</div>';
  
  h += '<div style="background:#0a0a0a;border-radius:12px;padding:16px;margin-bottom:16px">';
  h += '<h4 style="color:#a855f7;margin-bottom:8px">📍 Endereço</h4>';
  h += '<p>' + p.endereco.rua + ', ' + p.endereco.numero + '</p>';
  if (p.endereco.complemento) h += '<p>Complemento: ' + p.endereco.complemento + '</p>';
  h += '<p>' + p.endereco.bairro + ', ' + p.endereco.cidade + '</p>';
  if (p.endereco.referencia) h += '<p>Ref: ' + p.endereco.referencia + '</p>';
  h += '</div>';
  
  h += '<div style="background:#0a0a0a;border-radius:12px;padding:16px;margin-bottom:16px">';
  h += '<h4 style="color:#a855f7;margin-bottom:8px">📦 Itens</h4>';
  ...
  h += '<div style="background:#0a0a0a;border-radius:12px;padding:16px;margin-bottom:16px">';
  h += '<h4 style="color:#a855f7;margin-bottom:8px">💰 Pagamento</h4>';
  var pagNome = { dinheiro: 'Dinheiro', pix: 'PIX', 'cartao-credito': 'Cartao Credito', 'cartao-debito': 'Cartao Debito' };
  h += '<p><strong>Forma:</strong> ' + (pagNome[p.pagamento] || p.pagamento) + '</p>';
  if (p.troco) h += '<p><strong>Troco para:</strong> R$ ' + p.troco + '</p>';
  h += '<p><strong>Subtotal:</strong> R$ ' + p.total.toFixed(2) + '</p>';
  h += '<p><strong>Entrega:</strong> ' + (p.entrega === 0 ? 'GRATIS' : 'R$ ' + p.entrega.toFixed(2)) + '</p>';
  h += '<p style="font-size:18px;color:#a855f7"><strong>TOTAL: R$ ' + p.totalFinal.toFixed(2) + '</strong></p>';
  h += '</div>';
  
  if (p.obs) {
    h += '<div style="background:#0a0a0a;border-radius:12px;padding:16px;margin-bottom:16px">';
    h += '<h4 style="color:#a855f7;margin-bottom:8px">📝 Observação</h4>';
    h += '<p>' + p.obs + '</p></div>';
  }
  
  h += '<h4 style="color:#a855f7;margin-bottom:12px">🔄 Alterar Status</h4>';
  h += '<div style="display:flex;flex-wrap:wrap;gap:8px">';
  var statuses = ['recebido','confirmado','em_preparo','no_forno','saiu_entrega','entregue','cancelado'];
  for (var s = 0; s < statuses.length; s++) {
    var ss = STATUS_PEDIDO[statuses[s]];
    var bg = p.status === statuses[s] ? '#581c87' : '#1a1a1a';
    var color = p.status === statuses[s] ? '#fff' : '#a1a1a1';
    h += '<button onclick="alterarStatus(\\'' + id + '\\',\\'' + statuses[s] + '\\')" style="padding:8px 16px;border-radius:8px;border:none;cursor:pointer;font-size:13px;background:' + bg + ';color:' + color + '">' + ss.icon + ' ' + ss.nome + '</button>';
  }
  h += '</div>';
...
function filterAdmin(filter, btn) {
  ...
  btn.style.background = '#581c87'; btn.style.color = '#fff';
  var pedidos = JSON.parse(localStorage.getItem('pizzaPedidos') || '[]');
...
# Validar chaves
import re
js_part = final[final.find('<script>')+8:final.find('</script>')]
open_b = js_part.count('{')
close_b = js_part.count('}')
print(f"Arquivo reconstruido: {len(final)} chars")
print(f"Chaves: {open_b} abertas, {close_b} fechadas")
if open_b == close_b:
    print("JS VALIDO!")
else:
    print(f"ERRO: Diferenca de {open_b - close_b} chaves")
